...
void ampPlayString(int iChannel, char *pString) {
	...
	while(ptr < max) {
		...
		if (c == 'O' || c == 'o') { octave = v; }
		else if (c == 'L' || c == 'l') { length = (v * 12 * 120) / tempo; }	// convert from our written format to the internal
		else if (c == 'V' || c == 'v') { volume = v; }	// convert from our written format to the internal
		else if (c == 'T' || c == 't') { tempo = v; }	// convert from our written format to the internal
		else if (c == 'R' || c == 'r') { note = -1; }
		else if (c >= 'A' && c <= 'G') { note = c-'A'; }
		else if (c >= 'a' && c <= 'g') { note = c-'a'; }

		switch(note) {
                 case -2:  // no note this time, just commands
                      break;

                 case -1:
                      anpPlayRestFor(iChannel, length);
                      break;
                  default:
